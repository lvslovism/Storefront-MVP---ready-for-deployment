...
// 發送設定
// Note: Resend 免費方案只能用 onboarding@resend.dev，設定自訂 domain 後再改回 noreply@minjie0326.com
const FROM_EMAIL = process.env.FROM_EMAIL || 'MINJIE STUDIO <onboarding@resend.dev>';
...
function getPasswordResetEmailTemplate(code: string, name: string): EmailTemplate {
  return {
      ...
      <td align="center">
        <table width="100%" style="max-width: 480px; background-color: #111111; border: 1px solid #D4AF37; border-radius: 12px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 32px;">
              <p style="margin: 0 0 16px; color: #FFFFFF; font-size: 16px;">親愛的 ${name}，您好！</p>
              <p style="margin: 0 0 24px; color: #AAAAAA; font-size: 14px;">我們收到了您的密碼重設請求。請使用以下驗證碼：</p>
              <!-- OTP Code -->
              <div style="background-color: #1A1A1A; border: 2px solid #D4AF37; border-radius: 8px; padding: 24px; text-align: center; margin-bottom: 24px;">
                <span style="font-size: 36px; font-weight: 700; color: #D4AF37; letter-spacing: 8px;">${code}</span>
              </div>
...
export async function sendPasswordResetEmail(
  ...
  const template = getPasswordResetEmailTemplate(code, name);
